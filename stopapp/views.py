...
def password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Password Update Successfull')
            return redirect('index')
        else:
    ...
    if request.method == 'POST':
        ...
        if cart:
            ...
            if basket:
                ...
                newbasket =ShopCart() #this runs when a new item is added to the basket
                newbasket.user = request.user
                newbasket.product = item
                newbasket.basket_no = cart[0].basket_no
...
def placeorder(request):
    if request.method == 'POST':
        api_key = 'sk_test_9a6be6afde7c1e4104c4819f11c344ce96aad110'
        curl = 'https://api.paystack.co/transaction/initialize'
        # cburl = 'http://localhost:8000/completed/'
        cburl = 'http://18.217.117.239/completed/'
        ...
        # collect data that you will send tp paystack
...
def increase(request):
    ...
    newquant = ShopCart.objects.get(pk=valid)
    newquant.quantity = itemval
    newquant.save()
